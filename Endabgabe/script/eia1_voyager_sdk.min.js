...
function handleTouch13() {
    ...
    const gifElement = document.querySelector('img');
     ...
     // Zufällige Änderung der Position des Bildes
    const maxX = window.innerWidth - gifElement.width; // Maximale horizontale Position
    const maxY = window.innerHeight - gifElement.height; // Maximale vertikale Position
    
    const randomX = Math.floor(Math.random() * maxX); // Zufällige horizontale Position
    const randomY = Math.floor(Math.random() * maxY); // Zufällige vertikale Position
    
    // Änderung der Position des Bildes
...
function handleTouch33() {
    // diese Statements werden ausgeführt, sobald PIN 33 aktiv ist
}
